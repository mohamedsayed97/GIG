...
namespace ICP_ABC.Areas.Policies.Controllers
{
    public class PolicyController : Controller
    {
        ...
        public ActionResult Index()
        {
            var model = new PolicyViewModel
            {
                ...
                Companies = dbContext.Companies.Where(c => c.Auth).ToList()
            };
            return View(model);
        }
        ...
        public ActionResult Edit(string PolicyNo)
        {
            ...
            if(model == null || model.EffectiveDate.Date <= DateTime.Now.Date)
            {
...
BusinessChannel
      */
        ...
        public ActionResult Details(string PolicyNo)
        {
            ...
            model.EditBtn = true;
            var policy = dbContext.Policy.Include("AllocationRules").SingleOrDefault(p => p.PolicyNo == PolicyNo);
            ...
            model.Companies = dbContext.Companies.Where(c => c.Auth).ToList();
            ...
            model.HasBooster = policy.HasBooster;
            model.HasWithdrawal = policy.HasWithdrawal;
            model.AllocationRules = policy.AllocationRules.Select(ar => new AllocationRuleViewModel()
            {
                FundId = ar.FundId,
                PercentageOfCompanyShare = ar.PercentageOfCompanyShare,
                PercentageOfCompanyShareBooster = ar.PercentageOfCompanyShareBooster,
